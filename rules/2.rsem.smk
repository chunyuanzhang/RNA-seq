...
rule make_tx2tene_rsem:
    input:
        gff = lambda wildcards: get_genome_gff(wildcards.species) 
    output:
        tx2gene = "result/RSEM/{species}_trans2symbol.tsv"
    params:
        Species = "{species}",
        MappingTool = "rsem",
        outdir = "result/RSEM/"
...
if design == "Species":
    rule DESeq_rsem_between_species:
        input:
            isoforms = expand("result/RSEM/{sample}.isoforms.results", sample = samples),
            Orthologgenes = Orthologgenes
        output:
            all_genes = "result/RSEM/{pairname}.all.tsv",
            diffexp_genes = "result/RSEM/{pairname}.diffexp.tsv"
        params:
            infotable = infotable,
            design = design,
            lfc = lfc,
    ...
    rule DESeq_rsem_within_species:
        input:
            isoforms = expand("result/RSEM/{sample}.isoforms.results", sample = samples)
        output:
            all_genes = "result/RSEM/{pairname}.all.tsv",
