...
pdf("result/WGCAN/Chicken.模块相关性.pdf", width = 4, height = 4)
...
pdf("result/WGCAN/Duck.模块相关性.pdf", width = 4, height = 4)
...
ggsave(filename = "result/WGCNA/Chicken.preservation.pdf", plot = p.pre.chicken, width = 6, height = 4)
...
ggsave(filename = "result/WGCNA/WGCAN/Duck.preservation.pdf", plot = p.pre.duck, width = 6, height = 4)
...
for (module in genes_in_module.chicken %>% names()) {
  ...
  enrichment_GO.chicken[[module]] <- GO_enrichment(
    ...
    backgroundgenes = extract_gene_from_gene_vs_gene_pair(assay(dds) %>% rownames(), side = 1),
...
for (module in genes_in_module.duck %>% names()) {
  ...
  enrichment_GO.duck[[module]] <- GO_enrichment(
    ...
    backgroundgenes = extract_gene_from_gene_vs_gene_pair(assay(dds) %>% rownames(), side = 2),
...
for (module in genes_in_module.chicken %>% names()) {
  ...
  ggsave(filename = paste0("result/WGCAN/Chicken.",module,".anno.pdf" ), plot = p, width = 14, height = 6)
}
...
for (module in genes_in_module.duck %>% names()) {
  ...
  p <- ggpubr::ggarrange(p.GO, p.KEGG, labels = c("A", "B"), ncol = 2)
  ggsave(filename = paste0("result/WGCAN/Duck.",module,".anno.pdf" ), plot = p, width = 14, height = 6)
}
...
saveRDS(WGCNAfile, file = "result/WGCNA/WGCNAfile.rds")
