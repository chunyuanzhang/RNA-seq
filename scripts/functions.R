...
get_protein2gene <- function(gff, protein = TRUE){
  ...
  p2g <- gff_df %>%
    mutate(
      GENEID = stringr::str_match(attr, "GeneID:([0-9]+)")[,2],              # 稳定数字ID
      SYMBOL = stringr::str_match(attr, "(?:^|;)gene=([^;]+)")[,2],           # 顺带留 symbol
      PROTEINID = stringr::str_match(attr, "(?:^|;)protein_id=([^;]+)")[,2]
    ) %>% 
